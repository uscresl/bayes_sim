...
class Gaussian:
    ...
    def __init__(self, m=None, P=None, U=None, S=None, Pm=None, L=None):
        ...
        :param L: lower triangular covariance factor given as 1D array such that LL' = S
        """
...
class Gauss(object):
    ...
    def eval(self, xs, ii=None, log=True):
        if ii is None:
            if not log:
                result = [norm.pdf(x=x, loc=self.mu, scale=self.sigma)[0] for x in xs]
            else:
                result = [norm.logpdf(x=x, loc=self.mu, scale=self.sigma) for x in xs]

        else:
            lp = scipy.stats.multivariate_normal.logpdf(xs, self.mu, self.sigma)
            lp = np.array([lp]) if xs.shape[0] == 1 else lp
            result = lp if log else np.exp(lp)
...
class MoG:
    ...
    def gen(self, n_samples=1):
        ...
        samples = [x.gen() for x, n in zip(self.xs, ns)]
        weighted_samle = [sample * weight for sample, weight in zip(samples, self.a)]
        sample = sum(weighted_samle)
        return sample
    ...
    def eval(self, x, ii=None, log=True):
        ...
        ps = np.array([self.xs[ix].eval(x, ii, log) for ix in range(len(self.a))]).T
...
def fit_mog(x, n_components, w=None, tol=1.0e-9, maxiter=float('inf'), verbose=False):
    ...
    while True:
        ...
        # calculate log p(x,z), log p(x) and total log likelihood
        logPxz = np.array([scipy.stats.multivariate_normal.logpdf(x, ms[k], Ss[k])
                           for k in range(n_components)])
        logPxz += np.log(a)[:, np.newaxis]
        logPx = scipy.misc.logsumexp(logPxz, axis=0)
